...
export function setupWalkthrough(
  ...
  mainBtn.className =
    'px-3 py-1.5 text-xs font-semibold rounded-md transition-all bg-indigo-600 text-white';

  const allBtn = document.createElement('button');
  allBtn.textContent = 'All Features';
  allBtn.className =
    'px-3 py-1.5 text-xs font-semibold rounded-md transition-all text-slate-400 hover:text-white';
  ...
  if (toggleQaBtn) {
    toggleQaBtn.onclick = () => {
      if (qaContainer) {
        const isVisible =
          window.getComputedStyle(qaContainer).display !== 'none';
        qaContainer.style.display = isVisible ? 'none' : 'block';
        toggleQaBtn.textContent = isVisible
          ? 'Open Feature Walkthrough'
          : 'Close Feature Walkthrough';
        if (!isVisible) {
          updateQaPosition();
          renderQA();
        }
